package com.team5.librarydatabasemanage.views.removestudent;
...
@Route(value = "my-view18", layout = MainLayout.class)
public class RemoveStudentView extends Composite<VerticalLayout> {
    ...
    public RemoveStudentView() {
        ...
        formLayout3Col2.setResponsiveSteps(new ResponsiveStep("0", 1), new ResponsiveStep("250px", 2),
                new ResponsiveStep("500px", 3));
        select.setLabel("Select Student");
        select.setWidth("min-content");
        setSelectStudentData(select);
        buttonPrimary.setText("Remove Student");
        ...
        buttonPrimary.addClickListener(event -> {
            String studentID = select.getValue().toString();
            String studentIDValue = extractID(studentID);

            String deleteQuery = "DELETE FROM Student WHERE Student_ID = " + studentIDValue + ";";
            try {
                ConnectDatabase.statement.executeUpdate(deleteQuery);
                Notification.show("Student removed successfully");
            } catch (SQLException e) {
                Notification.show("Student remove failed: " + e.getMessage());
                e.printStackTrace();
            }
            setSelectStudentData(select);
        });
    ...
    private void getSelectListBoxRemoveStudentDatas(List<String> items) {
        ...
        try {
            ...
            while (resultSet.next()) {
                studentID.add(resultSet.getInt("Student_ID"));
        ...
        for (int i = 0; i < studentID.size() && i < studentName.size(); i++) {
            items.add("ID: " + studentID.get(i) + ", Name: " + studentName.get(i));
        }
    }

    private void setSelectStudentData(Select<String> select) {
        List<String> studentItems = new ArrayList<>();
        getSelectListBoxRemoveStudentDatas(studentItems);
        select.setItems(studentItems);
    }
