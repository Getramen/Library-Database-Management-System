package com.team5.librarydatabasemanage.views.removepublisher;
...
@PageTitle("Remove Publisher")
@Route(value = "my-view16", layout = MainLayout.class)
public class RemovePublisherView extends Composite<VerticalLayout> {

    public RemovePublisherView() {
        ...
        formLayout3Col2.setResponsiveSteps(new ResponsiveStep("0", 1), new ResponsiveStep("250px", 2),
                new ResponsiveStep("500px", 3));
        select.setLabel("Select Publisher");
        select.setWidth("min-content");
        setSelectPublisherData(select);
        buttonPrimary.setText("Remove Publisher");
        ...
        buttonPrimary.addClickListener(event -> {
            String publisherID = select.getValue().toString();
            String publisherIDValue = extractID(publisherID);

            String deleteQuery = "DELETE FROM Publisher WHERE Publisher_ID = " + publisherIDValue + ";";
            try {
                ConnectDatabase.statement.executeUpdate(deleteQuery);
                Notification.show("Publisher removed successfully");
            } catch (SQLException e) {
                Notification.show("Publisher remove failed: " + e.getMessage());
                e.printStackTrace();
            }
            setSelectPublisherData(select);
        });
    ...
    private void getSelectListBoxRemovePublisherDatas(List<String> items) {
        List<Integer> publisherID = new ArrayList<>();
        List<String> publisherName = new ArrayList<>();

        try {
            String query = "SELECT Publisher_ID, Name FROM view_Publisher;";
        ...
        for (int i = 0; i < publisherID.size() && i < publisherName.size(); i++) {
            items.add("ID: " + publisherID.get(i) + ", Name: " + publisherName.get(i));
        }
    }

    private void setSelectPublisherData(Select<String> select) {
        List<String> publisherItems = new ArrayList<>();
        getSelectListBoxRemovePublisherDatas(publisherItems);
