package com.team5.librarydatabasemanage.views.addauthor;
...
public class AddAuthorView extends Composite<VerticalLayout> {
    ...
    public AddAuthorView() {
        ...
        formLayout3Col2.setResponsiveSteps(new ResponsiveStep("0", 1), new ResponsiveStep("250px", 2),
                new ResponsiveStep("500px", 3));
        textField.setLabel("Author ID");
        textField.setWidth("min-content");
        textField2.setLabel("Author Name");
        textField2.setWidth("min-content");
        textField3.setLabel("Author Surname");
        textField3.setWidth("min-content");
        buttonPrimary.setText("Add Author");
        ...
        buttonPrimary.addClickListener(event -> {
            String authorID = textField.getValue();
            String authorName = textField2.getValue();
            String authorSurname = textField3.getValue();

            String insertQuery = "INSERT INTO Author (Author_ID, Author_name, Author_surname) VALUES ('"
                    + authorID + "','" + authorName + "','" + authorSurname + "');";

            try {
                ConnectDatabase.statement.executeUpdate(insertQuery);
                Notification.show("Author Added successfully");
            } catch (SQLException e) {
                Notification.show("Author add failed: " + e.getMessage());
                e.printStackTrace();
            }
        });
